...
procedure insertInBinaryTree(abr : in out Arbre; valeur : in T) is
begin
    if isEmptyBinaryTree(abr) then
...
function searchInBinaryTree(abr : in Arbre; valeur : in T) return Boolean is
begin
    if isEmptyBinaryTree(abr) then
        return false;
    else
...
procedure modifyInBinaryTree(abr : in out Arbre; src : in T; tar : in T) is
begin
    deleteInBinaryTree(abr, src);
    insertInBinaryTree(abr, tar);
end modifyInBinaryTree;

procedure deleteInBinaryTree(abr : in out Arbre; valeur : in T) is
 -- Permet de trouver la plus petite valeur dans l'arbre
 function minimumBinaryTree(abr : in Arbre) return T is
 begin
