generic
    ...
    with function "=" (Item1, Item2 : in T) return Boolean;
    with function "<" (Item1, Item2 : in T) return Boolean;
    with function ">" (Item1, Item2 : in T) return Boolean;
package AB is
...
-- Exception : aucune
procedure insertInBinaryTree(abr : in out Arbre; valeur : in T) with Post=> searchInBinaryTree(abr,valeur) = True;
...
procedure deleteInBinaryTree(abr : in out Arbre; valeur : in T) with Post=> searchInBinaryTree(abr,valeur) = False;
...
procedure modifyInBinaryTree(abr : in out Arbre; src : in T; tar : in T) with Post=> searchInBinaryTree(abr,tar) ;
...
-- Post-conditions : un arbre avec l'élément présent
-- Exception : aucune
function searchInBinaryTree(abr : in Arbre; valeur : in T) return Boolean;
